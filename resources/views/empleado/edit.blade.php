...
<div class="container-fluid px-4">
  ...
  <div class="card card-custom mt-4">
    <form action="{{ route('empleados.update', $empleado) }}"
      ...
      <div class="col-lg-7 p-4">
        ...
        <div class="mb-3 form-floating">
          <input
            ...
            value="{{ old('razon_social', $empleado->razon_social) }}"
            required>
      ...
      <div class="col-lg-5 p-4 border-start d-flex flex-column align-items-center">
        <h6 class="mb-3 text-center">Vista previa</h6>
        <div class="img-preview-container mb-4">
          @if($empleado->img && Storage::disk('public')->exists($empleado->img))
          <img id="img-preview" src="{{ Storage::url($empleado->img) }}" alt="Imagen actual">
          @else
          <img id="img-preview" src="{{ asset('assets/img/R.png') }}" alt="Sin imagen">
          @endif
        </div>

        <button type="submit" class="btn btn-primary w-100 mb-2">Actualizar</button>
        <a href="{{ route('empleados.index') }}" class="btn btn-outline-secondary w-100">Cancelar</a>
      </div>
    </form>
  </div>
</div>

...
class roleController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            DB::commit();
             ActivitylogService::log('Creacion de rol', 'roles', $request->all());
             return redirect()->route('roles.index')->with('success', 'Rol registrado');
        } catch (Throwable $e) {
            DB::rollBack();
             Log::error("Error al crear el rol", ['error' => $e->getMessage()]);
            return redirect()->route('roles.index')->with('error', 'Ups, algo fallo');
        }
    ...
    public function update(Request $request, Role $role):RedirectResponse
    {
        ...
        try {
            ...
            DB::commit();
             ActivitylogService::log('Edicion de rol', 'roles', $request->all());
             return redirect()->route('roles.index')->with('success', 'Rol editado');
        } catch (Throwable $e) {
            DB::rollBack();
             Log::error("Error al editar el rol", ['error' => $e->getMessage()]);
            return redirect()->route('roles.index')->with('error', 'Ups, algo fallo al editar');
        }
    ...
    public function destroy(string $id): RedirectResponse
    {
        try {
            ...
            ActivitylogService::log('Eliminacion de rol', 'roles', ['rol_id' => $id]);
             return redirect()->route('roles.index')->with('success', 'Rol eliminado');
        } catch (Throwable $e) {
            Log::error("Error al eliminar el rol", ['error' => $e->getMessage()]);
            return redirect()->route('roles.index')->with('error', 'Ups, algo fallo al eliminar');
        }
